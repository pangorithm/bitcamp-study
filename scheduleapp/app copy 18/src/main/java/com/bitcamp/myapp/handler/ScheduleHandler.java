...
public class ScheduleHandler implements Handler {
  ...
  static Schedule tempSchdule = new Schedule();
  ...
  private void inputSchedule() {
    this.list.add(inputScheduleInfo());
  }

  private void printSchedules() {
    for (Schedule sch : this.list.list()) {
  ...
  private void updateSchedule() {
    ...
    Schedule sch = this.list.get((inputNo));
    if (sch == null) {
      ...
      printScheduleInfo(sch);
      System.out.println("---------------------");
      list.update(inputScheduleInfo(), inputNo);
    }
  }

  private void deleteSchdule() {
    int memberNo = this.prompt.inputInt("번호? ");
    Schedule sch = this.list.get(memberNo);
    if (sch == null) {
    ...
    if (this.prompt.inputString("정말로 이 스케줄을 삭제 하시겠습니까?(y/N)").equalsIgnoreCase("y")) {
      list.delete(memberNo);
    }
  }

  private Schedule inputScheduleInfo() {
    Schedule sch = tempSchdule;
