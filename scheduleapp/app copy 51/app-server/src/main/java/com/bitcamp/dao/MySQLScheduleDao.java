...
public class MySQLScheduleDao implements ScheduleDao {
  ...
  public MySQLScheduleDao(DataSource ds) {
    this.ds = ds;
  }
  ...
  public int remove(Schedule schedule) {
    try (PreparedStatement stmt = ds.getConnection(false)
        .prepareStatement("delete from scheduleapp_schedule where schedule_no=? and owner=?");) {
  ...
  public int scheduleAddParticipant(int scheduleNo, int memberNo) {
    try (PreparedStatement stmtSelect =
        ds.getConnection(false).prepareStatement("select * from scheduleapp_schedule_participants"
      ...
      try (ResultSet rs = stmtSelect.executeQuery()) {
        if (!rs.next()) {
          try (PreparedStatement stmtInsert = ds.getConnection().prepareStatement(
