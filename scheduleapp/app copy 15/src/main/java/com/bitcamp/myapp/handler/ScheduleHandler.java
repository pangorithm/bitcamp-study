...
public class ScheduleHandler {
  ...
  Prompt prompt;
  private Schedule[] schedules = new Schedule[MAX_SIZE];
  private int length = 0;
  static String str;

  public ScheduleHandler(Prompt prompt) {
    this.prompt = prompt;
  }

  public void inputSchedule() {
    do {
      inputScheduleInfo(this.length);
      this.length++;
      str = this.prompt.inputString("일정을 계속 추가 하시겠습니까?(y/N) \n> ");

    } while (this.prompt.promptContinue(str));
  }
  ...
  public void updateSchedule() {
    int inputNo = this.prompt.inputInt("번호? ");
  ...
  private void inputScheduleInfo(int index) {
    Schedule sch = this.schedules[index] == null ? new Schedule() : this.schedules[index];
    ...
    this.schedules[index] = sch;
  }

  private void printScheduleInfo(int index) {
    Schedule sch = this.schedules[index];
  ...
  private int indexOf(int inputNo) {
    for (int i = 0; i < this.length; i++) {
      if (this.schedules[i].getNo() == inputNo) {
        return i;
      }
    }
    return -1;
