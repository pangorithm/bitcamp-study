...
public class ApplicationContext {
  ...
  private void processBeanAnnotation(Class<?> configClass) throws Exception {
    ...
    for (Method m : methods) {
      ...
      Bean beanAnnotaion = m.getAnnotation(Bean.class);
      ...
      if (beanAnnotaion.value().length() > 0) {
        // 애노테이션에 객체 이름이 지정되어 있다면 그 이름으로 객체를 저장한다.
        beanContainer.put(beanAnnotaion.value(), returnValue);
      } else {
  ...
  private void createBeans(String basePackage) throws Exception {
    ...
    String dirPath = basePackage.replaceAll("[.]", "/");

    // 2) 클래스 경로를 관리하는 객체를 준비한다.
    ClassLoader systemClassLoader = ClassLoader.getSystemClassLoader();

    // 3) 클래스로더에게 패키지 경로에 해당하는 디렉토리 정보를 읽을 수 있는 도구를 달라고 요구한다.
    InputStream dirInputStream = systemClassLoader.getResourceAsStream(dirPath);
    ...
    // .lines() // 오리지널 데이터 스트림을 줄 단위 데이터로 나열한 스트림으로 변환한다.
